  ...
  return (
    <div className={`${isMobile? 'h-auto w-full flex-col px-[1rem] py-[2rem]' : 'h-screen w-[96%] flex-col gap-[1rem] p-[2rem]'}
        ...
        <div className={`${isMobile?  'flex-col' : 'flex-row'}
            ...
            <div className='h-[12rem] flex-1 flex flex-col items-start justify-start rounded-xl bg-[var(--highlight-color)] text-[var(--light-color)] gap-[1rem] p-[1rem]'>
                ...
                <div className='h-auto flex-2 flex flex-row items-start justify-start'>
                    <span className='font-semibold text-[1rem] cursor-pointer'>Vask rapidly attracting a substantial lazada user based of over 10,000 customers within its first year of operation.</span>
                </div>        
            </div>
            {isMobile && <div className='flex flex-row items-center justify-center flex-1'>
                <img src={Loved} className='h-[18rem] w-[18rem]' alt="" />
            </div>}
            <div className='h-[12rem] flex-1 flex flex-col items-start justify-start rounded-xl bg-[var(--primary-color)] text-[var(--light-color)] gap-[1rem] p-[1rem]'>
                <div className='h-auto flex-2 flex flex-row items-start justify-start'>
                    <span className={`${isMobile? 'text-[2rem] text-center' : 'text-[3rem]'} font-bold cursor-pointer`}>98%</span>
                </div>
                <div className='h-auto flex-2 flex flex-row items-start justify-start'>
                    <span className='font-semibold text-[1rem] cursor-pointer'>Users loved how it makes their life easier in terms of vehicle monitoring.</span>
                </div> 
            </div>
            {isMobile && <div className='flex flex-row items-center justify-center flex-1'>
                <img src={Downloads} className='h-[18rem] w-[18rem]' alt="" />
            </div>}
            <div className='h-[12rem] flex-1 flex flex-col items-start justify-start rounded-xl bg-[var(--secondary-color)] text-[var(--light-color)] gap-[1rem] p-[1rem]'>
                <div className='h-auto flex-2 flex flex-row items-start justify-start'>
